...
Usage (example):
  python send_and_receive_parallel.py --port COM3 --baud 115200 --in input/myimage.png --out-dir output --width 128 --height 128 --grayscale 0

Notes:
 - Requires `pyserial` and `Pillow` (install with `pip install pyserial pillow`).
 - By default expects RGB output (3 bytes/pixel). Use `--grayscale 1` for 1 byte/pixel outputs.
 - You can enable `--send-header` to prefix a small header: b'IMG' + width(2) + height(2) + size(4).
"""
import argparse
import os
import sys
...
def main():
    ...
    # We send RGB payload (width*height*3) and expect grayscale (width*height) back
    expected = width * height
    ...
    recv_thread = threading.Thread(target=receiver_thread_fn, args=(ser, out_bin, log_csv, expected, grayscale, stop_event, done_event, TIMEOUT_IDLE), daemon=True)
    recv_thread.start()
    ...
    try:
        reconstruct_image(out_bin, out_img, width, height, grayscale=True)
