...
def receiver_thread_fn(ser, out_bin_path, log_csv_path, expected_bytes, grayscale, stop_event, done_event, timeout_idle, perf_csv_path):
    ...
    After image reception, continues to receive performance logs.
    """
    buf = bytearray()
    ...
    # Image reception complete - now receive performance logs
    print("\n--- Performance Log Reception ---")
    log_buf = bytearray()
    log_start_time = time.time()
    log_timeout = 5.0
    
    while (time.time() - log_start_time) < log_timeout and not stop_event.is_set():
        try:
            data = ser.read(ser.in_waiting or 1)
            if data:
                log_buf.extend(data)
                print(f"Received {len(data)} log bytes (total: {len(log_buf)})")
                
                # Check if we have complete log (LOG: ... END\n)
                if b'LOG:' in log_buf and b'END\n' in log_buf:
    ...
    if len(log_buf) > 0:
        log_start = log_buf.find(b'LOG:')
        if log_start >= 0 and log_start + 6 <= len(log_buf):
            entry_count = struct.unpack('>H', log_buf[log_start+4:log_start+6])[0]
            ...
            for i in range(entry_count):
                pos = entry_start + (i * 4)
                if pos + 4 <= len(log_buf):
                    entries.append(parse_log_entry(log_buf[pos:pos+4]))
            ...
            if entries:
                ...
                print(f"✓ Saved performance logs to {perf_csv_path}")
    else:
        print("No performance log data received")
...
def main():
    ...
    recv_thread = threading.Thread(target=receiver_thread_fn, args=(ser, out_bin, log_csv, expected, OUT_GRAYSCALE, stop_event, done_event, TIMEOUT_IDLE, perf_csv), daemon=True)
    recv_thread.start()
