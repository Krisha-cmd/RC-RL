...
This variant expects a 64x64 RGB output (64*64*3 = 12,288 bytes).
"""
import argparse
import os
import sys
    ...
    with open(out_bin_path, 'wb') as fout, open(log_csv_path, 'w', newline='') as flog:
        ...
        while not stop_event.is_set():
            ...
            if data:
                ...
                if len(buf) % 1024 == 0 or len(buf) >= expected_bytes:
                    print(f"  Received {len(buf)}/{expected_bytes} bytes...")
            else:
                # no data read
                if expected_bytes and len(buf) >= expected_bytes:
                    print(f"  ✓ Received expected {expected_bytes} bytes, stopping...")
                    break
                # idle timeout
                if (time.time() - last_recv_time) > timeout_idle and len(buf) > 0:
                    # Assume transmission finished
                    print(f"  ⚠ Timeout after {len(buf)} bytes (expected {expected_bytes})")
                    break
            if expected_bytes and len(buf) >= expected_bytes:
                break
        # final flush
...
def main():
    ...
    PORT = 'COM4'
    BAUD = 115200
    INFILE = 'input/sample.jpeg'  # Test image with cyan top, yellow bottom
    OUT_DIR = 'output'
    WIDTH = 128
    HEIGHT = 128
    GRAYSCALE = False
    ...
    # Expected output dimensions (64x64 RGB)
    OUT_WIDTH = 64
    OUT_HEIGHT = 64
    OUT_GRAYSCALE = False  # RGB output
    ...
    # We send 128x128 RGB payload (width*height*3) and expect 64x64 RGB (64*64*3) back
    expected = OUT_WIDTH * OUT_HEIGHT * 3
